        ...
        try {
            ...
            foreach ($strutture as $remoteId => $struttura) {
                ...
                foreach ($data as $item) {
                    ...
                    $dipendenteRemoteId = $item['email'];
                    if (isset($dipendenti[$dipendenteRemoteId])) {
                        $dipendenti[$dipendenteRemoteId]['struttura'][] = $remoteId;
                    } else {
                        $dipendenti[$dipendenteRemoteId] = [
                            'struttura' => [$remoteId],
                            'data' => $item
                        ];
                    }
            ...
            foreach ($struttureOnLine as $strutturaOnLine) {
                ...
                if (!isset($strutture[$remoteId])) {
                    $struttureOnLineDaRimuovere[] = ['url' => "https://opencity.comune.bolzano.it/openpa/object/{$remoteId}", 'descrizione' => $strutturaOnLine->attribute('name')];
                } else {
            ...
            foreach ($strutture as $struttura) {
                $struttureDaAggiungere[] = ['codice' => $struttura['codiceIride'], 'descrizione' => $struttura['descrizioneIt']];
            }
            ...
            foreach ($dipendenti as $dipendente) {
                if (!isset($dipendente['data']['struttura']['descrizioneIt'])) {
                    $strutturaDiRiferimento = implode(',', array_column($dipendente['data']['struttura'], 'descrizioneIt'));
                } else {
                    $strutturaDiRiferimento = $dipendente['data']['struttura']['descrizioneIt'];
                }
                $dipendentiDaAggiungere[$dipendente['data']['cognome'] . $dipendente['data']['nome'] . $dipendente['data']['codice']] = ['codice' => $dipendente['data']['codice'], 'nome' => $dipendente['data']['cognome'] . ' ' . $dipendente['data']['nome'], 'email' => $dipendente['data']['email'], 'struttura' => $strutturaDiRiferimento];
            }
            ...
            if (count($dipendentiOnLineDaRimuovere)) {
                ...
                $message .= '<table width="100%" border="1" cellspacing="0" cellpadding="4">';
                $message .= '<tr><th width="1">Codice</th><th>Nome</th><th>Email</th></tr>';
                foreach ($dipendentiOnLineDaRimuovere as $item) {
                    $message .= '<tr><td>' . $item['codice'] . '</td><td><a href="' . $item['url'] . '">' . $item['nome'] . '</a></td><td>' . $item['email'] . '</td></tr>';
                }
                $message .= '</table>';
            }
            $message .= '</body></html>';
